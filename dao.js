...
module.exports = function (model, schema) {
  ...
  // limit of 0 means all values, else absolute of limit is used
  // point here is that no error is thrown
  this.findAndPopulate = function (filter, populate, orderby, skipCount, limit) {
    debug(`DB find method.filter :${JSON.stringify(filter)}, populate:${populate}, orderby :${JSON.stringify(orderby)}, skipCount :${skipCount}, limit :${limit} are parameters`);
    return this.objectModel.find(filter)
      .populate(populate)
      .sort(orderby)
      .skip(skipCount) // skipCount should not be negative
